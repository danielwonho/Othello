...
public class MonteCarloPlayer extends OthelloPlayer{
	...
	/*
	 * Monte carlo tree policy function.
	 * if there are children, return the last child
	 * if the node is terminal, just return the node
	 * otherwise just choose the best child or a random child (sometimes)
	 */
	private MCNode treePolicy(MCNode node){
		if(node==null)return null;//System.out.println("HI");
	...
	private OthelloState defaultPolicy(MCNode node){
		...
		do{
	        ...
	        if (!moves.isEmpty()) moveToMake = moves.get(r.nextInt(moves.size()));
