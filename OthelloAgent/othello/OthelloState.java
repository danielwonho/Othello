...
public class OthelloState {
    ...
    public OthelloState(int a_boardSize) {
        ...
        if (boardSize<2) boardSize = 2; 		//2x2 board is minimum
    ...
    /*
     * Returns the final score, once a game is over
     * added a 2nd function for playing as player 2
     */
    public int score2() {
    	int score = 0;
        for(int i = 0;i<boardSize;i++)
     ...
     * Returns the final score, once a game is over
     * NEW HEURISTIC
     * Takes corners captured and number of moves available into account
     */
    public int score3() {
    	int score = 0;
        for(int i = 0;i<boardSize;i++)
        ...
        if (board[7][7]==PLAYER1) score++;
        if (board[0][7]==PLAYER1) score++;
        if (board[7][0]==PLAYER1) score++;
        if (board[0][0]==PLAYER1) score++;
        if (board[7][7]==PLAYER2) score--;
        if (board[0][7]==PLAYER2) score--;
        if (board[7][0]==PLAYER2) score--;
        if (board[0][0]==PLAYER2) score--;
        if (generateMoves(PLAYER1).size()>generateMoves(PLAYER2).size())score++;
        else if (generateMoves(PLAYER2).size()>generateMoves(PLAYER1).size())score--;
        return score;
    }
    
    /*
     * Returns the final score, once a game is over
     * NEW HEURISTIC for playing on other side
     * Takes corners captured and number of moves available into account
     */
    public int score4() {
    	int score = 0;
        ...
        if (board[7][0]==PLAYER1) score--;
        if (board[0][0]==PLAYER1) score--;
        if (generateMoves(PLAYER2).size()>generateMoves(PLAYER1).size())score++;
        else if (generateMoves(PLAYER1).size()>generateMoves(PLAYER2).size())score--;
        return score;
    }
